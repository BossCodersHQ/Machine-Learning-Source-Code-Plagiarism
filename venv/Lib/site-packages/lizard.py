...
def print_and_save_modules(all_fileinfos, scheme):
    ...
    for module_info in saved_fileinfos:
        data.append([module_info.filename,
                     module_info.nloc,
                     module_info.average_nloc,
                     module_info.average_cyclomatic_complexity,
                     module_info.average_token_count,
                     len(module_info.function_list)])
        # print((
        #     "{module.nloc:7d}" +
        #     scheme.average_formatter() +
        #     "{function_count:10d}" +
...
def print_result(result, option, scheme, total_factory):
    result, data = print_and_save_modules(result, scheme)
...
def main(argv=None):
    ...
    """
    sys.stdout = open("logs/lizard_log.txt", "w+")   #hides output
    ...
    result = analyze(
        ...
        options.extensions,
        options.languages)
    warning_count, data = printer(result, options, schema, AllResult)
    print_extension_results(options.extensions)
    list(result)
    # if 0 <= options.number < warning_count:
    #     sys.exit(1)
    sys.stdout = sys.__stdout__ # reset back to using stdout
    return data
